...
class RestaurantOrderManagement:
    def __init__(self, root):
        ...
        # Set up the background image (must be PNG or GIF for PhotoImage)
        self.bg_width, self.bg_height = 800, 600
        self.canvas = tk.Canvas(root, width=self.bg_width, height=self.bg_height)
        self.canvas.pack(fill="both", expand=True)
        try:
            # Use images1.jpeg as background (convert to PNG if needed)
            self.original_image = tk.PhotoImage(file="images1.jpeg")  # Use PNG version of images1.jpeg
            subsample_x = max(1, self.original_image.width() // self.bg_width)
            subsample_y = max(1, self.original_image.height() // self.bg_height)
            self.background_image = self.original_image.subsample(subsample_x, subsample_y)
            self.canvas.create_image(0, 0, anchor=tk.NW, image=self.background_image)
            self.canvas.image = self.background_image
        except Exception as e:
            print("Background image error:", e)

        # Create a frame for widgets on top of canvas
        frame = ttk.Frame(root)
        frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

        ttk.Label(frame, text="Restaurant Order Management", font=("Arial", 20, "bold")).grid(
        ...
        currency_dropdown.grid(
            row=len(self.menu_items) + 1, column=1, padx=10, pady=5
        )
